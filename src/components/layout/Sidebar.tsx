...
export const Sidebar: React.FC = () => {
  return (
    <aside className="w-[240px] shrink-0 bg-card border-r border-border flex flex-col h-full">
      <div className="h-14 flex items-center px-5 border-b border-border gap-2.5">
        ...
        <div>
          <p className="text-sm font-display font-bold text-foreground leading-tight tracking-tight">Experiment</p>
          <p className="text-[10px] text-muted-foreground leading-tight tracking-wider uppercase">Proxy Portal</p>
        </div>
      </div>

      <nav className="flex-1 p-3 overflow-y-auto">
        <p className="px-3 pt-3 pb-1.5 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Navigation</p>
        <div className="space-y-0.5">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2 rounded-[var(--radius)] text-sm transition-all duration-150 no-underline group ${
                  isActive
      ...
      <div className="p-4 border-t border-border">
        <p className="text-[11px] text-muted-foreground font-medium">v1.0.0 — Enterprise</p>
      </div>
    </aside>
  );
};
