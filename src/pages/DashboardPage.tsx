    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card padding="md">
          ...
          <div className="space-y-3">
            <button className="w-full text-left px-4 py-3 bg-secondary border border-border hover:bg-accent rounded-[var(--radius)] transition-colors duration-150 group">
              <p className="text-sm font-medium text-foreground">Create New Experiment</p>
              <p className="text-xs text-muted-foreground mt-1">Set up a new A/B test or feature flag</p>
            </button>
            <button className="w-full text-left px-4 py-3 bg-secondary border border-border hover:bg-accent rounded-[var(--radius)] transition-colors duration-150">
              <p className="text-sm font-medium text-foreground">Add Connector</p>
              <p className="text-xs text-muted-foreground mt-1">Connect to external services and APIs</p>
            </button>
            <button className="w-full text-left px-4 py-3 bg-secondary border border-border hover:bg-accent rounded-[var(--radius)] transition-colors duration-150">
              <p className="text-sm font-medium text-foreground">Configure Mapping</p>
              <p className="text-xs text-muted-foreground mt-1">Map data between systems</p>
            </button>
          </div>
        </Card>
      </div>
